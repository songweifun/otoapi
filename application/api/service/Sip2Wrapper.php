...
Loader::import('Sip2.sip2', EXTEND_PATH, '.class.php');
...
class Sip2Wrapper {
    ...
    public function tushchaxun($ao,$ab,$ac){
        $this->_sip2->AO=$ao;
        $this->_sip2->AC=$ac;
        $msg=$this->_sip2->msgItemInformation($ab);
        $status= $this->_sip2->parseItemInfoResponse($this->_sip2->get_message($msg));

//        if ( $status['fixed']['CirculationStatus'] != true) {
//            throw new Sip2ConfigException([
//                'msg'=>'图书不存在或者查询失败',
//                'errorCode'=>30002
//            ]);
//        }
    ...
    public function duzhechaxun($ao,$aa,$ac,$ad,$bp,$bq){
        $this->_sip2->AO=$ao;
        $this->_sip2->patron=$aa;
        $this->_sip2->AC=$ac;
        $this->_sip2->patronpwd=$ad;
        $msg=$this->_sip2->msgPatronInformation('none');
        return $this->_sip2->parseItemInfoResponse($this->_sip2->get_message($msg));

    }

    public function jieshu($ao,$aa,$ab,$ad,$ac){
        $this->_sip2->AO=$ao;
        $this->_sip2->patron=$aa;
        $this->_sip2->patronpwd=$ad;
        $this->_sip2->AC=$ac;
        $msg=$this->_sip2->msgCheckout($ab,time()+86400,'Y');
        $res=$this->_sip2->parseCheckoutResponse($this->_sip2->get_message($msg));
//        if ((int) $res['fixed']['Ok'] !== 1) {
//            throw new Sip2ConfigException([
//                'msg'=>'借书失败',
//                "code"=>400,
//                'errorCode'=>30003
...
//        }
    ...
    public function huanshu($ap,$ao,$ab,$ac,$bi){
        $this->_sip2->AO=$ao;
        $this->_sip2->AC=$ac;
        $msg=$this->_sip2->msgCheckin($ab,time());
        //echo $msg;
        $res= $this->_sip2->parseCheckinResponse($this->_sip2->get_message($msg));
        if ((int) $res['fixed']['Ok'] !== 1) {
            throw new Sip2ConfigException([
                'msg'=>'还书失败',
                "code"=>401,
